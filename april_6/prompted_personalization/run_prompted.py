"""
Run Gemma-3-4B-IT on GSM8K, MATH, SVAMP, ASDiv, DROP with student/expert
...
async def main():
    ...
    print(f"Personas: student, expert")
    ...
    print(f"{'═'*60}")
    print(f"  {'dataset':>8}  {'student':>10}  {'expert':>10}")
    print(f"  {'─'*8}  {'─'*10}  {'─'*10}")
    for ds_name in datasets:
        s = summaries.get(f"{ds_name}_student", {})
        e = summaries.get(f"{ds_name}_expert", {})
        sa = f"{s.get('correct','?')}/{s.get('total','?')}"
        ea = f"{e.get('correct','?')}/{e.get('total','?')}"
        print(f"  {ds_name:>8}  {sa:>10}  {ea:>10}")
